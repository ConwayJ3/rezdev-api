...
const FROM = process.env.EMAIL_FROM || 'RezDev <notifications@rezdevos.com>';
const FROM_ADDRESS = process.env.EMAIL_FROM_ADDRESS || 'notifications@rezdevos.com';

// The recipient should see the BUILDER's name, not ours — they have a
// relationship with the builder and none with RezDev. One verified sending
// domain, many display names.
function buildSender(companyName){
  const name = (companyName || '').replace(/["<>]/g, '').trim();
  return name ? (name + ' <' + FROM_ADDRESS + '>') : FROM;
}

// Replies should reach a person. Callers pass the builder's address where
// they have it; without one the reply goes nowhere useful, same as before.
function sendMail(opts, companyName, replyTo){
  const payload = Object.assign({}, opts, { from: buildSender(companyName) });
...
async function sendClientInvite({ to, clientName, builderName, companyName, setupUrl, role, replyTo }){
...
<body style="margin:0;padding:0;background:#f4f6f8;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;">
  ...
  return sendMail({ to, subject, html }, companyName, replyTo);
}
...
async function sendContractNotification({ to, clientName, builderName, companyName, projectName, replyTo }){
  if(!to) return null;
  const company = companyName || 'your builder';
...
async function sendMessageNotification({ to, recipientName, senderName, projectName, companyName, replyTo }){
  if(!to) return null;
...
async function sendRfpInvite({ to, companyName, builderName, projectName, rfpTitle, deadline, bidUrl, replyTo }){
  if(!to) return null;
...
async function sendBidReceived({ to, builderName, rfpTitle, contractorName, companyName, amount, projectName, replyTo }){
  if(!to) return null;
...
async function sendBidDeclined({ to, contractorName, companyName, rfpTitle, projectName, replyTo }){
  if(!to) return null;
...
async function sendBidAwarded({ to, contractorName, companyName, rfpTitle, projectName, amount, setupUrl, replyTo }){
  if(!to) return null;
  const company = companyName || 'your builder';
...
// Ask a subcontractor to sign a lien waiver for what they've been paid.
async function sendLienWaiverRequest({ to, contractorName, companyName, projectName, amount, throughDate, replyTo }){
  if(!to) return null;
  const company = companyName || 'your builder';
  const subject = `Lien waiver requested${projectName ? ' \u2014 ' + projectName : ''}`;
  const amountText = (amount != null && amount !== '')
    ? '$' + Number(amount).toLocaleString() : null;
  const lines = [
    `Hi ${contractorName || 'there'},`,
    `${company} has requested a lien waiver${projectName ? ' for ' + projectName : ''}.`,
  ];
  if(amountText) lines.push(`Amount: ${amountText}`);
  if(throughDate) lines.push(`Covering work through ${throughDate}.`);
  lines.push('Sign in to your contractor portal to review and sign it.');
  const html = notificationHtml({
    subLabel: 'Lien waiver',
    heading: 'A lien waiver needs your signature',
    bodyLines: lines,
