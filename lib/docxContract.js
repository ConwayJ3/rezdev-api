...
function applyTagsToDocx(docxBuffer, rules){
  ...
  rules.forEach(rule => {
    // globalText contains ONLY <w:t> run text — no newlines (paragraph breaks are markup).
    // Trim any leading/trailing whitespace/newlines the tagger may have captured,
    // otherwise indexOf() can never match and the rule silently does nothing.
...
function applySignatureAnchors(docxBuffer, hasClient2){
  ...
  // SignWell assigns each {{signature:N}} to the Nth recipient by POSITION (not id).
  // Recipients are sent in order: Client 1 (pos 1), [Client 2 (pos 2) if present],
  // Builder (last). So the builder is position 2 when solo, position 3 with a co-signer.
  if(!hasClient2){
    // No co-signer: strip Client 2 markers; builder is the 2nd (and last) signer.
    ['##SIG_CLIENT2##','##DATE_CLIENT2##'].forEach(marker => {
