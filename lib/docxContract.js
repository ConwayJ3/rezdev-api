...
function applySignatureAnchors(docxBuffer){
  ...
  // First, plain-replace the markers with the SignWell tags.
  map.forEach(([marker, tag]) => { xml = xml.split(marker).join(tag); });

  // Then make each SignWell tag's text WHITE so it's invisible on the page
  // (SignWell places the field at the tag but does not remove the tag text).
  // We recolor the run(s) that contain a {{signature:n}} or {{date:n}} tag by
  // injecting a white color property into that run's <w:rPr>.
  // Match a run <w:r>...<w:t>...{{signature:1}}...</w:t>...</w:r> and add color.
  xml = xml.replace(/<w:r\b([^>]*)>([\s\S]*?)<\/w:r>/g, (full, attrs, inner) => {
    if(!/\{\{(signature|date):\d+\}\}/.test(inner)) return full;
    // Ensure there's an rPr with white color
    if(/<w:rPr>/.test(inner)){
      if(/<w:color\b/.test(inner)){
        inner = inner.replace(/<w:color\b[^\/]*\/>/, '<w:color w:val="FFFFFF"/>');
      } else {
        inner = inner.replace('<w:rPr>', '<w:rPr><w:color w:val="FFFFFF"/>');
      }
    } else {
      inner = '<w:rPr><w:color w:val="FFFFFF"/></w:rPr>' + inner;
    }
