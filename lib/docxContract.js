...
function applySignatureAnchors(docxBuffer, hasClient2){
  ...
  // When there is NO co-signer, strip the Client 2 markers entirely so the
  // template still works for single-client projects. A {{signature:3}} tag with
  // no matching recipient would make SignWell reject the document.
  if(!hasClient2){
